...
export default {
  ...
  apply(ctx: any) {
    ...
    try {
      const settings = (ctx as any).get?.('settings');
    ...
    try {
      ctx.on('session/event', (session: unknown, event: any) => {
        ...
        if (String(event?.type).startsWith('assistant/chunk')) {
          ...
          if ((log as any).n === undefined) (log as any).n = 0;
          (log as any).n++;
          if ((log as any).n <= 8 || (log as any).n % 2000 === 0) log(`chunk#${(log as any).n} ${raw.slice(0, 260)}`);
          return;
        }
        ...
        const sid = String((session as { id?: string })?.id ?? '?').slice(-6);
        const detail =
    ...
    try {
      ctx.on('agent/inbox/claimed', (agent: unknown, message: unknown, turn: unknown) => {
        const m = message as { source?: { kind?: string } } | undefined;
        const t = turn as { id?: string } | undefined;
        log(`claimed agent=${(agent as { id?: string })?.id} src=${m?.source?.kind ?? '?'} turn=${t?.id ?? '?'}`);
      });
