import { z } from 'zod';
import type { DefectType, ReviewVerdict } from '../types.js';
 ...
 */
export function reviewVerdictJsonSchema(): Record<string, unknown> {
...
export function mergeShardedVerdicts(shards: ReviewVerdict[]): ReviewVerdict {
  const worst = (a: DefectType, b: DefectType): DefectType => {
    const rank: Record<DefectType, number> = { ambiguous: 0, execution: 1, plan: 2 };
  ...
  for (const s of shards) {
    for (const issue of s.issues) {
      let id = issue.id;
      if (seen.has(id)) id = `${id}-chunk${issues.length}`;
      seen.add(id);
      issues.push({ ...issue, id });
    }
  }
  const pass = shards.every((s) => s.pass);
  const defect = shards.reduce<DefectType>((acc, s) => worst(acc, s.pass ? 'ambiguous' : s.defect_type), 'ambiguous');
