...
export interface SystemPromptSection {
  section?: (def: { name: string; order: number; text: (context: unknown) => string }) => unknown;
}
...
export interface SettingsService {
  ...
  describe?: (opts?: { redactSecrets?: boolean }) => unknown;
}
...
export interface MechanicalSpawner {
  run: (changedFiles: string[], force?: boolean, opts?: { signal?: AbortSignal }) => Promise<MechanicalResult> | MechanicalResult;
}
