...
const effortEnum = Schema.union(['off', 'low', 'medium', 'high', 'max'] as const);
...
const stageSchema = Schema.object({
  ...
  on_failure: Schema.union(['hard_fail', 'auto_degrade'] as const).default('auto_degrade'),
...
export const orchestratorSettingsSchema: any = Schema.object({
  mode: Schema.union(['auto', 'passthrough', 'gated'] as const).default('auto')
...
export function buildConfigBase(cfg: OrchestratorConfig): OrchestratorSettingsSection {
  const pick = (s: StageConfig | undefined): Partial<StageConfig> | undefined =>
...
const STAGE_ENTRIES: Array<{ key: 'plan' | 'execute' | 'review'; effort: StageConfig['reasoning_effort']; onFailure: StageConfig['on_failure'] }> = [
...
function normalizeAuditList(list: StageConfig[] | undefined): StageConfig[] {
  return (list ?? [])
 ...
 */
export function applySettingsOverlay(base: OrchestratorConfig, section: OrchestratorSettingsSection): OrchestratorConfig {
  const next = structuredClone(base);
  if (section.mode) next.mode = section.mode;
  if (section.budget) {
  ...
  const leafSet = (dst: StageConfig, field: keyof StageConfig, value: unknown) => {
    if (value === undefined) return;
    const cleared = value === '' || value === null;
    if (cleared) delete (dst as Record<string, unknown>)[field as string];
    else (dst as Record<string, unknown>)[field as string] = value;
  };
  ...
  if (next.stages) {
    for (const { key } of STAGE_ENTRIES) {
      const ov = ss[key];
      if (!ov) continue;
      for (const f of ['model', 'provider', 'reasoning_effort', 'on_failure'] as const) {
        leafSet(next.stages[key], f, ov[f]);
      }
      if (Array.isArray(ov.fallback_chain)) next.stages[key].fallback_chain = [...ov.fallback_chain];
      else if (ov.fallback_chain === null) next.stages[key].fallback_chain = [];
    }
    if (Object.prototype.hasOwnProperty.call(ss, 'plan_audit')) {
      const clean = normalizeAuditList(ss.plan_audit);
      if (clean.length > 0) next.stages.plan_audit = clean;
      else delete next.stages.plan_audit;
    }
    // a section that removes every stage's model restores the inert state
    if (!next.stages.plan.model || !next.stages.execute.model || !next.stages.review.model) next.stages = undefined;
    ...
    const mk = (ov: Partial<StageConfig>, effort: StageConfig['reasoning_effort'], onFailure: StageConfig['on_failure']): StageConfig => ({
      model: ov.model!,
    ...
    next.stages = {
      ...
      review: { ...mk(ss.review, 'max', 'auto_degrade'), dimensions: 'full' as const, input_token_budget: 50_000 },
      ...(audits.length ? { plan_audit: audits } : {}),
    } as OrchestratorConfig['stages'];
  }
  return next;
}
