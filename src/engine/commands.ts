/**
 * `/orch` + `/per` command registration (docs/重构方案-v3.md §2-D).
 *
 * The two command families plus their per-session override bookkeeping. The
 * only things that cross back into the core are `launchTask` (a forced
 * orchestration start, which is `handleUserInput` with a one-shot flag) and
 * `cancelTask` (the single `user/cancel` transition of the command layer — it
 * is defined in engine.ts so the FSM call graph stays in one file).
 */
import type { CommandRegistry } from '../host-contract.js';
import type { EngineCtx } from './ctx.js';

export function registerCommands(ctx: EngineCtx): void {
  const commands = ctx.host.get('commands') as CommandRegistry | undefined;
  const register = commands?.register;
  if (!register) return;
  const def = (name: string, description: string, hint: string, handler: (a: { agent: any; rawInput: string }) => unknown) =>
    register({ name, description, input: { hint }, handler });
  ...
  def('per', '编排执行模式快捷开关与一键启动', '<任务文本> | <on|off|auto>', ({ agent, rawInput }) => {
    ...
    if (input === 'on' || input === 'off' || input === 'auto') {
      if (input === 'auto') ctx.sessionModeOverride.delete(sessionId);
      else {
        ...
        if (ctx.sessionModeOverride.size > 5000) ctx.sessionModeOverride.clear();
  ...
  def('orch', '多模型编排控制', '<status|set|reset|save|budget|task|passthrough|abort> [args]', ({ agent, rawInput }) => {
    ...
    switch (sub) {
      ...
      case 'save': {
        ...
        const merged = { ...(JSON.parse(ctx.fsOps.readTextIfExists(f) ?? '{}') as Record<string, string>), ...mine };
      ...
      case 'task': {
        ...
        if (!goal) return { kind: 'error', text: '用法: /orch task <目标>（强制进编排，绕过分流）' };
        ctx.oneShot.set(sessionId, { forced: true });
      ...
      case 'abort': {
        const t = ctx.registry.activeForSession(sessionId);
        if (!t) return { kind: 'error', text: '没有进行中的编排任务' };
