/**
 * Budget accounting (docs/重构方案-v3.md §2-D).
 *
 * `resolvePricingRate` / `isUnpricedModel` were already exported from
 * engine.ts (test/units.test.ts imports them) — engine.ts re-exports both so
 * the public surface is unchanged.
 *
 * `accountUsage` performs the debit, the token/call breaker checks and the
 * unpriced-model warning, then *reports* the transition it wants instead of
 * firing it: returning the event name keeps every `transition()` call site in
 * engine.ts while the accounting decision lives next to the ledger it drives.
 */
 ...
 */
export const BUILTIN_PRICING: Record<string, { input: number; output: number }> = {
...
export interface PricingConfigLike {
  ...
  pricing_unknown?: { input: number; output: number } | null;
}

/** Effective rate lookup: `budget.pricing` > built-in table > `budget.pricing_unknown` > null (¥0). */
export function resolvePricingRate(cfg: PricingConfigLike, model: string): { input: number; output: number } | null {
...
export function isUnpricedModel(cfg: PricingConfigLike, model: string): boolean {
...
export function accountUsage(
  ...
  const cfg: OrchestratorConfig = ctx.cfg();
  ...
  if (model !== 'unknown' && isUnpricedModel(cfg.budget, model)) {
    ctx.noticeOnce(
      sessionId,
      `unpriced:${model}`,
      `【编排】模型 ${model} 未配置单价，本次按 ¥0 记账——日/任务限额对它不生效。请在补丁配置中设置 budget.pricing.${model}（或 budget.pricing_unknown 兜底）。`,
    );
  }
  if (child) {
    ...
    if (snap) {
      ...
      const res = ledger.debit(
        child.taskId,
        child.role === 'reviewer' ? 'review' : child.role === 'executor' ? 'execute' : 'plan',
        binding?.provider,
        model,
        usage.inputTokens,
        usage.outputTokens,
      ...
      if (t && !res.ok) return 'budget/exhausted';
    }
    return null;
  }
  if (!task || isTerminal(task.state)) {
    if (passthroughObserve.has(sessionId)) ledger.observePassthrough(model, usage.inputTokens, usage.outputTokens);
    return null;
  }
  ...
  if (!res.ok) return 'budget/exhausted';
