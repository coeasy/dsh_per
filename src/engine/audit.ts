...
import {
  buildAuditPrompt,
  buildAuditFeedbackDirective,
  latestAudit,
  mergeAuditVerdicts,
  planAuditVerdictJsonSchema,
  planAuditVerdictSchema,
  type MergedAudit,
  type PlanAuditVerdict,
} from '../protocols/plan-audit.js';
import { isTerminal, type PlanAuditRound } from '../types.js';
...
export function bindAuditor(
  ctx: EngineCtx,
  cfg: { model: string; provider?: string; fallback_chain: string[] },
): { model: string; provider?: string } | null {
  if (!ctx.health.isFailed(cfg.provider, cfg.model)) return { model: cfg.model, provider: cfg.provider };
  for (const fb of cfg.fallback_chain) {
    if (!ctx.health.isFailed(undefined, fb)) return { model: fb };
  }
  return null;
}
 ...
 */
export async function dispatchPlanAuditors(
  ctx: EngineCtx,
  task: OrchestratorTask,
  agent: AnyAgent,
  signal: AbortSignal,
  cfgs: NonNullable<OrchestratorConfig['stages']>['plan_audit'],
  previous: PlanAuditRound | null,
  response: PlanAuditResponse | null,
): Promise<{ merged: MergedAudit; skipped: number } | null> {
  const runtime = ctx.subagentRuntime();
  if (!runtime?.start) {
    task.snapshot.lastReviewError = 'subagents service missing (plan-audit)';
    return null;
  }
  const realCfgs = (cfgs ?? []) as Array<{ model: string; provider?: string; reasoning_effort: string; fallback_chain: string[] }>;
  interface Spawned {
    idx: number;
    model: string;
    run: any;
  ...
  for (const [idx, cfg] of realCfgs.entries()) {
    ...
    try {
      ...
      if (!run) throw new Error('subagents.start returned no run handle');
      if (run.id) {
  ...
  const settled = await Promise.all(
    spawned.map(async ({ idx, run }) => {
      const result = await withTimeoutDispose(run?.result, () => run?.dispose?.(), timeoutMs, ctx.logger);
      if (result === null || result?.stopReason !== 'completed' || !result?.structured) return null;
      ...
      return { idx, verdict: parsed.data as PlanAuditVerdict };
    }),
  );
  const verdicts = settled.filter((s): s is { idx: number; verdict: PlanAuditVerdict } => Boolean(s)).map((s) => s.verdict);
  if (verdicts.length === 0) return null;
  const skipped = spawned.length - verdicts.length;
  if (skipped > 0) ctx.logger.warn('orchestrator: %d/%d plan auditor(s) produced no verdict (partial degradation)', skipped, spawned.length);
 ...
 * `fire` callback.
 */
export async function runAuditGateIfPending(
  engine: EngineCtx,
  fire: TransitionFn,
  task: OrchestratorTask,
  agent: AnyAgent,
  ...
  if (!task.pendingPlanAudit || auditCfgs.length === 0) return false;
  if (isTerminal(task.state)) return true;
  ...
  const st = task.state as 'PLANNING' | 'RE-PLANNING';
  const okEvent = st === 'RE-PLANNING' ? ('replan/ok' as const) : ('plan/ok' as const);
  const failEvent = st === 'RE-PLANNING' ? ('replan/fail' as const) : ('plan/fail' as const);
  ...
  const recordRound = (merged: MergedAudit, passed: boolean) => {
    snap.planAudit.push({
