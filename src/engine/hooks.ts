...
import type { OrchestratorTask, TaskHooks } from '../task/fsm.js';
...
export function buildHooks(ctx: EngineCtx): TaskHooks {
  const hooks: TaskHooks = {
    handoffBudgetCheck: (task) => {
      const plan = task.snapshot.plan;
      if (!plan) return;
      const tokens = estimateTokens(JSON.stringify(plan));
      if (tokens > plan.estimated_context_tokens * 0.8) task.snapshot.planDigested = true;
    },
