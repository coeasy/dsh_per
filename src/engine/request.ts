...
export async function onRequest(engine: EngineCtx, fire: TransitionFn, payload: any, next: () => Promise<any>) {
  const { agents, requestRetries, lastBinding, registry, router, health, sessionOverrides } = engine;
  const agent: AnyAgent = payload.agent;
  const sessionId = agent?.id;
  const resolved = await next();
  if (!sessionId) return resolved;
  agents.set(sessionId, agent);
  if (requestRetries.get(sessionId)) requestRetries.delete(sessionId); // a request succeeded
  ...
  const changed =
    (binding.provider && binding.provider !== resolved.provider) ||
...
export async function onRequestError(engine: EngineCtx, fire: TransitionFn, payload: any, next: () => Promise<unknown>) {
  const { requestRetries, lastBinding, registry, router, health, sessionOverrides } = engine;
  const sessionId = (payload.agent as AnyAgent | undefined)?.id;
  const provider: string | undefined = payload.provider;
  const failure = payload.failure as { code: string; message: string; status?: number } | undefined;
  if (sessionId) {
