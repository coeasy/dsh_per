...
export async function onSessionEvent(engine: EngineCtx, fire: TransitionFn, session: any, event: any) {
  ...
  if (event.type === 'assistant/message') {
    ...
    if (st === 'EXECUTING') {
      if (text.includes('[NEED_REPLAN]')) task.snapshot.needReplan = true;
    }
    return;
  }
  ...
  if (event.type === 'turn/start' && task && !isTerminal(task.state)) {
    ...
    if (task.state === 'EXECUTING' && !child) task.snapshot.execErrorCount = 0;
  ...
  if (event.type === 'tool/result' && task && !isTerminal(task.state)) {
    ...
    if (isError && !child) task.snapshot.execErrorCount++;
  ...
  if (event.type === 'step/end' && task && !isTerminal(task.state)) {
    ...
    if (child?.role !== 'auditor') task.snapshot.stageSteps[stage]++;
    ...
    if (task.snapshot.executionLog.length > logMax) task.snapshot.executionLog = task.snapshot.executionLog.slice(-Math.ceil(logMax / 2));
  }
}
