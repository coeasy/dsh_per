 ...
 */
export class Gate {
  private passthroughRes: RegExp[];
  private orchestrateRes: RegExp[];

  constructor(private cfg: GateConfig, private onInvalid: (message: string) => void = () => {}) {
  ...
  private compileAll(kind: 'passthrough_patterns' | 'orchestrate_patterns', patterns: string[]): RegExp[] {
    const compiled: RegExp[] = [];
    for (const pattern of patterns) {
      try {
        compiled.push(new RegExp(pattern, 'u'));
      } catch (e) {
        this.onInvalid(`orchestrator: dropping invalid ${kind} entry ${JSON.stringify(pattern)}: ${(e as Error).message}`);
      }
    }
    return compiled;
  }

  decide(input: { text: string; forced?: boolean; passthroughFlag?: boolean }): GateOutcome {
    const text = input.text ?? '';
    if (input.forced) return { decision: 'orchestrate', forced: true, rule: '1: forced flag (/orch task)' };
    if (input.passthroughFlag) return { decision: 'passthrough', forced: false, rule: '2: passthrough flag (/orch passthrough)' };
