 ...
 */
export function extractFencedPlan(text) {
    const re = /```plan\s*\n([\s\S]*?)```/g;
    let last = null;
...
export function parsePlan(fencedJson) {
    if (fencedJson === null)
        return { ok: false, error: 'no ```plan fenced block found' };
    let raw;
    try {
    ...
    return {
        ...
        ...(d.audit_response ? { auditResponse: d.audit_response } : {}),
    };
}
/**
...
export function estimateTokens(text) {
    ...
    for (const ch of text) {
        if (ch.codePointAt(0) > 0x2e7f)
            cjk++; // CJK ranges + fullwidth punctuation
    }
