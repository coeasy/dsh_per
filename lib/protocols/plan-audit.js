...
export function planAuditVerdictJsonSchema() {
    return {
        type: 'object',
...
const SEV_RANK = { minor: 0, major: 1, blocker: 2 };
/** Conservative-union arbitration across 1-2 auditor verdicts (D1). */
export function mergeAuditVerdicts(verdicts) {
...
export function latestAudit(history) {
...
export function buildAuditPrompt(args) {
    ...
    if (args.previous) {
        parts.push(`【上轮审计问题】${JSON.stringify(args.previous.issues)}`, `【规划者响应】采纳：${JSON.stringify(args.response?.adopted ?? [])}；反驳：${JSON.stringify(args.response?.rebutted ?? [])}`, `【本轮职责】逐条验证：① 已采纳问题是否真正修复；② 反驳是否成立（在 rebuttal_verdicts 给出 accepted 与 note）；③ 按四维重新扫查新版计划。问题沿用稳定 id（IA1、IA2…），已结案的 id 不得再出现。`);
    }
    parts.push(`【输出】严格通过 structured_output 提交 PlanAuditVerdict JSON：{passed, confidence, issues:[{id,dimension,severity,description,location?,suggestion}], rebuttal_verdicts:[{id,accepted,note}]}。`, `【规则】不确定或证据不足时 passed=false 并降低 confidence；blocker/major 必须给出 suggestion；没有问题则 issues 为空、passed=true。`);
    return parts.join('\n\n');
}
/** Rewrite directive back to the planner carrying the negotiation record (round r→r+1). */
export function buildAuditFeedbackDirective(args) {
...
//# sourceMappingURL=plan-audit.js.map