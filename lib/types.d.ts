 ...
 */
export type TaskState = 'IDLE' | 'PLANNING' | 'PLAN_FAIL' | 'EXECUTING' | 'RE-PLANNING' | 'REVIEWING' | 'DONE' | 'DONE_FLAGGED' | 'ABORTED';
export declare const TERMINAL_STATES: readonly TaskState[];
export declare function isTerminal(state: TaskState): boolean;
export type TaskEventName = 'task/launch' | 'plan/ok' | 'plan/fail' | 'plan/steps-exceeded' | 'plan/retry' | 'exec/ok' | 'exec/ok:DONE' | 'exec/need-replan' | 'exec/fail' | 'exec/fail:REPLAN' | 'exec/steps-exceeded' | 'exec/defect:EXHAUSTED' | 'replan/ok' | 'replan/fail' | 'replan/fail:EXHAUSTED' | 'review/pass' | 'review/pass:MISS' | 'review/fail-exec' | 'review/fail-exec:DONE' | 'review/fail-exec:ABORT' | 'review/fail-plan' | 'review/fail-plan:DONE' | 'review/ambiguous' | 'review/deadlock' | 'review/deadlock:EXEC' | 'budget/exhausted' | 'budget/exhausted:FLAGGED' | 'user/cancel' | 'circuit/broken' | 'model/hard-fail';
export type CounterName = 'plan_retry' | 'exec_retry' | 'replan_cycle' | 'fix_cycle' | 'review_supplement' | 'review_dispatch_retry';
...
export type IssueDimension = 'plan_conformance' | 'code_quality' | 'boundary' | 'security' | 'test_coverage';
 ...
 * exist.
 */
export type AbortReason = 'guard_chain_overflow' | 'guard_rejected' | 'stale_snapshot';
export declare const COUNTER_DEFAULTS: Counters;
