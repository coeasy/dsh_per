...
export function createNotify(deps) {
    ...
    const notice = (sessionId, text) => {
        ...
        if (cfg().visibility.progress === 'quiet' && !isTerminalNotice(text))
            return;
    ...
    const noticeOnce = (sessionId, key, text) => {
    ...
    const directive = (sessionId, text) => {
        const agent = agents.get(sessionId);
        const msg = createUserMessage({
            content: [{ type: 'text', text }],
