...
import { mergeShardedVerdicts, reviewVerdictJsonSchema, reviewVerdictSchema, routeVerdict, } from '../protocols/review.js';
...
export async function dispatchReviewPrompt(ctx, task, agent, signal, prompt, model, provider) {
    ...
    try {
        ...
        task.snapshot.lastReviewError = `spawn: ${String(e?.message ?? e).slice(0, 300)}`;
        return null;
    }
    if (run?.id) {
...
export async function dispatchReviewer(ctx, task, agent, signal, supplementQuestion) {
    if (isTerminal(task.state))
        return 'deadlock'; // aborted mid-run: no ghost reviews
    const runtime = ctx.subagentRuntime();
    if (!runtime?.start) {
        task.snapshot.lastReviewError = 'subagents service missing';
        return 'deadlock';
    ...
    if (hardFail)
        return 'hardfail'; // core fires model/hard-fail
    ...
    const needsSharding = (product?.changedFiles.length ?? 0) > SHARD_FILE_THRESHOLD || (product?.diffLines ?? 0) > SHARD_DIFF_THRESHOLD;
    let mergedVerdict = null;
    if (needsSharding && !supplementQuestion) {
        const files = product.changedFiles;
        const shards = Array.from({ length: Math.min(MAX_SHARDS, Math.ceil(files.length / SHARD_FILE_THRESHOLD) || 1) }, () => []);
        ...
        const shardRuns = shards
            .filter((s) => s.length > 0)
            .map((scopeFiles) => dispatchReviewPrompt(ctx, task, agent, signal, buildReviewPrompt(task, ctx.cfg(), previous, undefined, scopeFiles), binding.model, provider));
        const verdicts = (await Promise.all(shardRuns)).filter((v) => v !== null);
        if (verdicts.length >= 2)
            mergedVerdict = mergeShardedVerdicts(verdicts);
        else if (verdicts.length === 1)
            mergedVerdict = verdicts[0]; // E6: single shard is better than a wasted full-scope re-dispatch
    }
...
function snapSupplementUsed(task) {
 ...
 */
export async function runReview(engine, fire, task, agent, signal) {
    const { notice, pendingSteer, spawner } = engine;
    if (isTerminal(task.state))
        return 'ended';
    // P2-20: deadlock re-dispatch used to recurse; a bounded loop instead.
    for (let hops = 0; hops < 8; hops++) {
        const outcome = await dispatchReviewer(engine, task, agent, signal);
        if (isTerminal(task.state))
            return 'ended'; // a hook aborted the task during dispatch
        if (outcome === 'hardfail') {
    ...
    const mechanicalPass = checks.some((c) => c.check === 'unavailable')
        ? 'unavailable'
    ...
    notice(task.snapshot.sessionId, MILESTONES.reviewResult(false, verdict.issues.filter((i) => i.severity === 'blocker').length, verdict.issues.filter((i) => i.severity === 'major').length, verdict.issues.filter((i) => i.severity === 'minor').length));
    await fire(task, route);
    return 'steered';
}
