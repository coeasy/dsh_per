...
export function skeletonPlan(plan) {
    if (!plan)
        return '';
...
export function mechSummary(snap) {
    const m = snap.mechanicalResult;
    if (!m || (!m.compile && !m.lint && !m.tests))
        return '未运行';
    const fmt = (label, c) => c ? `${label}:${c.check}${c.summary ? `(${c.summary})` : ''}` : null;
...
export function replanOkMilestone(task) {
    return `【编排】新计划 v${task.snapshot.planVersion} 生效，重新开始执行`;
}
export function buildFixDirective(task, cfg) {
    ...
    const batchNote = cfg.fix_loop.strategy === 'batch'
        ? '（批量修复模式：按依赖关系一次性收敛全部问题，不要逐点反复返工）\n'
        : '';
    return `【修复指令】${batchNote}只修复以下问题点，保留其余成果，不做计划外改动：\n${items}${user}${escal}`;
}
export function buildReplanDirective(task, feedback) {
    return `【重规划】${feedback}\n请基于执行反馈修订计划，并重新输出完整 \`\`\`plan 围栏 JSON（作废旧计划 v${task.snapshot.planVersion}）。`;
}
export function buildFixDirectiveFromMechanical(task) {
 ...
 */
export function buildReviewPrompt(task, cfg, previous, supplementQuestion, scopeFiles) {
    ...
    const assemble = (planText, planNote) => {
        ...
        if (scopeFiles)
            parts.push(`【分片范围】本轮只复核以下文件的改动：${scopeFiles.join(', ')}`);
        if (previous) {
            parts.push(`【上一轮问题清单】（沿用未修复问题的 id，修复成功的问题不要再出现）\n${JSON.stringify(previous.issues.slice(-10))}`);
        }
        if (supplementQuestion)
            parts.push(`【补证请求】${supplementQuestion}`);
        parts.push(`【输出】严格输出 ReviewVerdict JSON：{planVersion, pass, defect_type: 'execution'|'plan'|'ambiguous', confidence: 0~1, issues: [{id, dimension, severity, description, location?}]}`, `【规则】confidence<0.7 或不确定时 defect_type 用 ambiguous；按 issue 粒度给出稳定 id（I1、I2…沿用上一轮编号）；minor 问题不阻塞交付。`);
        return parts.join('\n\n');
    };
    ...
    const planlessNote = dimensions === 'consistency_only'
