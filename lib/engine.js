...
import { mergeOverride, orchestratorConfigSchema } from './config/schema.js';
...
import { SETTINGS_NAMESPACE, applySettingsOverlay, buildConfigBase, orchestratorSettingsSchema, } from './settings/namespace.js';
import { OrchestratorTask } from './task/fsm.js';
import { TaskRegistry, TaskScheduler } from './task/registry.js';
import { MILESTONES, renderCost } from './visibility/milestones.js';
...
export function createEngine(ctx, rawConfig, deps) {
    ...
    const pricing = { rate: (_provider, model) => resolvePricingRate(effectiveConfig.budget, model) };
    ...
    const sessionModeOverride = new Map(); // /per on|off per-session; settled with tasks
    const sessionOverrides = new Map(); // /orch set per-session stage-model patches
    const passthroughObserve = new Set();
    const lastBinding = new Map();
    const requestRetries = new Map(); // sessionId → consecutive request-error retries
    const noticedOnce = new Map(); // sessionId → notice keys already shown
    const childSessions = new Map();
    const pendingSteer = new Map(); // taskId → directive text for next turn-stopping
    const disposers = [];
    // ── dependency seams (A1) ────────────────────────────────────────────────
    ...
    let savedRaw = (() => {
        try {
            return JSON.parse(fsOps.readTextIfExists(savedOverridesPath) ?? '{}') ?? {};
        }
        catch {
            return {};
        }
    ...
    const isOrchestrationEnabled = () => effectiveConfig.mode === 'gated' || (effectiveConfig.mode === 'auto' && Boolean(effectiveConfig.stages));
    /** Session-effective config: global layers + THIS session's /orch set overrides. */
    const sessionEffective = (sessionId) => {
    ...
    try {
        ctx.inject?.(['settings'], (services) => {
            const sctx = services;
            if (!sctx.settings)
                return; // provider absent: stay patch-only
            try {
                ...
                const rawUserOf = () => {
                    try {
                        const desc = sctx.settings.describe?.({ redactSecrets: true })?.find((d) => d.ns === SETTINGS_NAMESPACE);
                        return desc?.user ?? null;
                    }
                    catch {
                        return null;
                    }
                ...
                const unwatch = scope.watch(() => {
                    settingsSection = rawUserOf();
                    applyOverrides();
                    ctx.logger.info('orchestrator: settings section updated (mode=%s, stages=%s)', effectiveConfig.mode, Boolean(effectiveConfig.stages));
                });
    ...
    const stageOfState = (state) => state === 'EXECUTING' ? 'execute' : state === 'REVIEWING' ? 'review' : 'plan';
    ...
    const riskOf = (snap) => {
        if (snap.plan?.risk_level === 'high')
            return 'high';
        const hit = [...(snap.plan?.steps.flatMap((s) => s.files ?? []) ?? []), ...snap.executionProduct?.changedFiles ?? []].some((f) => sensitiveRes().some((re) => re.test(f)));
        if (hit)
            return 'high';
        if ((snap.executionProduct?.diffLines ?? 0) > effectiveConfig.risk_profile.high_diff_lines)
            return 'elevated';
        return 'standard';
    };
    // ── task settlement ───────────────────────────────────────────────────────
    ...
    const costLine = (task) => renderCost(task.snapshot, ledger.estimatedToday(), effectiveConfig.budget.daily_limit_cny);
    ...
    const hooks = buildHooks(engine);
    async function transitionOrLog(task, event) {
        let ok = false;
        try {
        ...
        if (!ok)
            ctx.logger.warn(`orchestrator: transition ${event} rejected on ${task.id} (state=${task.state})`);
        snapshots.save(task.snapshot);
        return ok;
    }
    // ── task creation & input routing (A10) ─────────────────────────────────
    async function handleUserInput(agent, message) {
        agents.set(agent.id, agent);
        requestRetries.delete(agent.id); // fresh input: reset the consecutive-failure cap
        noticedOnce.delete(agent.id);
        if (message?.source?.kind !== 'user')
            return; // A8: plugin sources never re-gate
        ...
        if (text.startsWith(REVIEW_PROMPT_MARKER) || text.startsWith(AUDIT_PROMPT_MARKER))
            return;
        if (registry.isChild(sessionId))
            return;
        ...
        let decision;
        if (modeOv === 'off') {
            decision = { decision: 'passthrough', forced: false, rule: 'mode: /per off（会话级）' };
        }
        else if (!isOrchestrationEnabled() && modeOv !== 'on') {
            decision = { decision: 'passthrough', forced: false, rule: 'mode: orchestration disabled (ADR #13)' };
        }
        else {
        ...
        if (decision.decision === 'passthrough') {
            ...
            if (passthroughObserve.size > 5000)
                passthroughObserve.clear();
        ...
        if (launched)
            notice(sessionId, `【编排】任务启动：${text.slice(0, 80)}（gate: ${decision.rule}）`);
    }
    // ── turn driver (V5: turn-stopping + steer) ─────────────────────────────
    async function onTurnStopping(payload) {
        ...
        for (let hops = 0; hops < 8; hops++) {
            switch (task.state) {
                case 'PLANNING': {
                    if (await runAuditGateIfPending(engine, transitionOrLog, task, agent, payload.signal))
                        return; // v5.2 gate
                    ...
                    const err = task.lastPlanError;
                    pendingSteer.set(task.id, `【规划指令】请完成调研后输出单个 \`\`\`plan 围栏 JSON 计划（PlanDocument schema：steps[{id,title,files?,risk_note?}], complexity, review_hint, risk_level, estimated_context_tokens）。${err ? `上次错误：${err}` : ''}`);
                    steerNext();
                    return;
                }
                case 'PLAN_FAIL': {
    ...
    disposers.push(ctx.on('agent/request', (payload, next) => onRequest(engine, transitionOrLog, payload, next)));
    disposers.push(ctx.on('agent/request-error', (payload, next) => onRequestError(engine, transitionOrLog, payload, next)));
    disposers.push(ctx.on('agent/turn-stopping', onTurnStopping));
    disposers.push(ctx.on('subagent/start', (info) => {
        if (info?.id)
            registry.trackChild(info.id);
    }));
